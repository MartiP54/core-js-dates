...
function getNextFriday(date) {
  const dateTemp = new Date(date);
  const dayTemp = 7 - dateTemp.getDay() + 5;
  if (dayTemp % 7 === 0) {
    dateTemp.setDate(dateTemp.getDate() + 7);
  } else {
    dateTemp.setDate(dateTemp.getDate() + (dayTemp % 7));
  }
  return dateTemp;
}
...
function getCountDaysInMonth(month, year) {
  const dateTemp = new Date(year, month, 0);
...
function isDateInPeriod(date, period) {
  ...
  const dateTemp = Date.parse(date);
  return dateTemp >= start && dateTemp <= end;
}
...
function formatDate(date) {
  const dataTemp = new Date(date);
  return `${dataTemp.toLocaleDateString('en-US', { timeZone: 'UTC' })}, ${dataTemp.toLocaleTimeString('en-US', { timeZone: 'UTC' })}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  dateTemp.setUTCFullYear(year, month, 0);
  let weekendCount = 0;
  for (let i = 1; i <= dateTemp.getDate(); i += 1) {
    const dateTemp2 = new Date();
    dateTemp2.setUTCFullYear(year, month - 1, i);
    if (dateTemp2.getDay() === 0 || dateTemp2.getDay() === 6) {
      weekendCount += 1;
    }
  }
  return weekendCount;
...
function getWeekNumberByDate(date) {
  ...
  const firstDayTempSec = Date.parse(firstDayTemp);
  return (
    Math.floor(
      ((dateTempSec - firstDayTempSec) / 86400000 +
        ((firstDayTemp.getUTCDay() + 6) % 7)) /
        7
    ) + 1
  );
}
...
function getNextFridayThe13th(date) {
  ...
  for (let m = 0; m < 12; m += 1) {
    const dateTemp2 = new Date();
    dateTemp2.setUTCFullYear(date.getFullYear(), m + 1, -1);
    for (let i = 1; i <= dateTemp2.getDate(); i += 1) {
...
function getQuarter(date) {
  let result = 0;
  if (Math.ceil(date.getMonth() / 3) === 0) {
    result = 1;
  } else {
    result = Math.ceil(date.getMonth() / 3);
  }
  return result;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  periodStart.setUTCFullYear(
    period.start.slice(6, 10),
    period.start.slice(3, 5) - 1,
    period.start.slice(0, 2)
  );
  periodEnd.setUTCFullYear(
    period.end.slice(6, 10),
    period.end.slice(3, 5) - 1,
    period.end.slice(0, 2)
  );
  workDay.push(
    new Date(periodStart).toLocaleDateString('en-GB').replace(/\//g, '-')
  );
  const tempDate = new Date(periodStart);
  while (tempDate < periodEnd) {
    tempDate.setUTCDate(tempDate.getUTCDate() + countWorkDays + countOffDays);
    workDay.push(
      new Date(tempDate).toLocaleDateString('en-GB').replace(/\//g, '-')
    );
  }
  workDay.pop();
  return workDay;
